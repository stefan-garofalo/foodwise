...
import { useIsMobile } from '@/modules/ui/hooks/media-query'

import { Button } from '@/modules/ui/primitives/button'

import { Separator } from '@/modules/ui/primitives/separator'
...
import { Skeleton } from '@/modules/ui/primitives/skeleton'
import {
	Tooltip,
	TooltipContent,
	TooltipProvider,
...
>(({ className, onClick, ...props }, ref) => {
	...
	return (
		<Button
			...
			variant="outline"
			size="icon"
			className={merge('h-7 w-7', className)}
			onClick={(event) => {
				onClick?.(event)
				toggleSidebar()
			}}
...
>(({ className, ...props }, ref) => {
	return (
		<Separator
			...
			className={merge('bg-sidebar-border mx-2 w-auto', className)}
			{...props}
		/>
	)
})
